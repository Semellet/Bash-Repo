...
errorlog=/var/log/updater_errors.log#!/bin/bash
# A basic one stop shop to update Linux based systems. Utilising search, standard input/output filtering, and functions to seemlessly update the active distro.

logfile=~/bash-scripts/log/updater.log
errorlog=~/bash-scripts/log/updater_errors.log
...
if [ ! -d ~/bash-scripts/log/ ]
then
	sudo mkdir ~/bash-scripts/log
	sudo touch $logfile $errorlog
	sudo chmod a+w $logfile $errorlog
fi
...
then
	...
	sudo apt-get update 1>$logfile 2>$errorlog && sudo apt-get dist-upgrade -y 1>>$logfile 2>>$errorlog
	check_exit_status
